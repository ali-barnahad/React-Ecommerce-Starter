...
// Dummy-Text für die Produktbeschreibung
const lorem =
...
const SingleProduct: FC = () => {
  ...
  // Effekt zum Abrufen der Produktinformationen
  useEffect(() => {
  ...
  // Effekt zum Abrufen ähnlicher Produkte basierend auf der Kategorie
  useEffect(() => {
  ...
  // Funktion zum direkten Kauf eines Produkts
  const buyNow = () => {
    requireAuth(() => {
      if (product)
        dispatch(
  ...
  // Funktion zum Hinzufügen eines Produkts zur Wunschliste
  const addWishlist = () => {
    requireAuth(() => {
      if (product) {
    ...
    <div className="container mx-auto pt-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 px-4 font-karla">
        ...
        <div className="px-2">
          ...
          {/* Schaltflächen zum Hinzufügen zum Warenkorb, Kauf und zur Wunschliste */}
          <div className="flex flex-wrap items-center mt-4 mb-2 space-x-2">
            <button
              type="button"
              className="flex items-center space-x-1 mb-2 hover:bg-pink-700 text-white text-xs text-sm p-2 rounded bg-pink-500"
              onClick={addCart}
            >
            ...
            <button
              type="button"
              className="flex items-center space-x-1 mb-2 bg-blue-500 text-white text-xs p-2 rounded hover:bg-blue-700 "
              onClick={buyNow}
            >
              <FaHandHoldingDollar />
              <span>BUY NOW</span>
            ...
            <button
              type="button"
              className="flex items-center space-x-1 mb-2 bg-yellow-500 text-white text-xs p-2 rounded hover:bg-yellow-700"
              onClick={addWishlist}
            >
              <MdFavoriteBorder />
